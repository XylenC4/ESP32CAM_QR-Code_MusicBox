...
void loop_INA226() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    TelnetStream.print("Bus voltage:   ");
    TelnetStream.print(INA226_BusVoltage, 5);
    TelnetStream.println(" V");

    TelnetStream.print("Bus power:     ");
    TelnetStream.print(INA226_BusPower, 5);
    TelnetStream.println(" W");

    TelnetStream.print("Shunt voltage: ");
    TelnetStream.print(INA226_ShuntVoltage, 5);
    TelnetStream.println(" V");

    TelnetStream.print("Shunt current: ");
    TelnetStream.print(INA226_ShuntCurrent, 5);
    TelnetStream.println(" A");

    if(INA226_ShuntCurrent > -0.1 && INA226_ShuntCurrent < 0.1 && INA226_BusVoltage > 4.1) {
      SelectChargingPalette(1);
    } else if(INA226_ShuntCurrent < -0.5) {
      SelectChargingPalette(2);
    } else if(INA226_BusVoltage < 3.4) {
      SelectChargingPalette(3);
    }
  }
}
